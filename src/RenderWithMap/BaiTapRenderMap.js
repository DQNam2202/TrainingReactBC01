...
export default class BaiTapRenderMap extends Component {
  dataFilm = [
    ...
    },
    {
      ...
      hinhAnh:
        "https://movie0706.cybersoft.edu.vn/hinhanh/batmanvssuperman.jpg",
      moTa:
    ...
    },
    {
      ...
      hinhAnh:
        "https://movie0706.cybersoft.edu.vn/hinhanh/avengers-infinity-war.jpg",
      moTa:
  ...
  renderFilm = () => {
    const arrJSX = this.dataFilm.map((film, index) => {
      return (
        <div className="col-4 mt-2" key={index}>
          <div className="card">
            {/* để chèn giá trị động cho thuộc tính nến ta sử dụng String Template */}
            <div style={{backgroundImage: `url(${film.hinhAnh}), url(https://piscum.photos/150/450)`, backgroundPosition:'center', backgroundSize:'cover'}}>
              <img
                // Khi load dữ liệu lên ta sẽ ẩn hình ảnh đi
                // Khi cào dữ liệu thì google sẽ dựa vào thẻ img chứ không dữ vào url
                style={{opacity: 0}}
                width="150px"
                height="450px"
            ...
            <div className="card-body">
              <h4 style={{ height: 50 }} className="card-title">
                {film.tenPhim.length > 50 ? (
                  ...
                  <p>{film.tenPhim.sub(0, 100)}</p>
                ) : (
                  <p>{film.tenPhim}</p>
                )}
              </h4>
              <div style={{ height: 80 }} className="card-text">
                {film.moTa.length > 100 ? (
                    //SubStr dùng để cắt chuỗi (begin,end)
                  <p>{film.moTa.substr(0, 100)}...Xem Thêm</p>
                ) : (
                  <p>{film.moTa}</p>
                )}
              </div>
  ...
  // Hiển thị danh sách film dưới dạng div card bootstap
  render() {
    return (
      <div className="container">
        <h3 className="text-center display-4">DANH SÁCH FILM</h3>
